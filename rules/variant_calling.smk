rule freebayes:
    ...
    log:
        "logs/{sample}/freebayes/{sample}.log"
    benchmark:
        "benchmarks/{sample}/freebayes/{sample}.tsv"
    container:
...
rule get_snpeff_db:
    output:
        archive = "resources/snpeff_db_GRCh38.86.tar.gz"
    container:
        "docker://quay.io/biocontainers/snpeff:5.1--hdfd78af_4"
    shell:
        """
        mkdir -p snpeff_data
        snpEff -Xmx4g download -dataDir snpeff_data -v GRCh38.86
        tar -czf {output.archive} snpeff_data/
        """
...
rule snpeff_annotate_vcf:
    input:
        vcf = "results/{sample}/{sample}.somatic.vcf",
        db = "resources/snpeff_db_GRCh38.86.tar.gz"
    output:
